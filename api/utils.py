...
def save_item(item, censored_ids=None):
    """
    ...
    if not visitor_queryset.exists():
        ...
        try:
            v = Visitor.objects.create(**item)
            if censored_ids and item["id_number"] in censored_ids:
                v.censored = True
                v.save()
                log.info(f"Censoring id_number {item['id_number']}")

            log.info("saving to db item")
        except Exception as e:
            log.error(f"Error when saving item to database {e}")
        ...
        if item["time_end"]:
            ...
            visitor.save()
            log.info(f"Updating date: {item['date']} is found in db")

    else:
        log.info("{0}, date: {1} is found in db, not saving".format(item["sha1"], item["date"]))
