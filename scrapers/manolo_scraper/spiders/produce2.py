...
class ProduceSpider2(ManoloBaseSpider):
    ...
    def initial_request(self, date):
        ...
        url = (
            ...
            f'iDisplayLength={display_end}&mDataProp_0=0&sSearch_0=&bRegex_0=false&bSearchable_0=true'
            ...
            'mDataProp_9=9&sSearch_9=&bRegex_9=false&bSearchable_9=true&mDataProp_10=10&sSearch_10=&'
    ...
    def parse(self, response, **kwargs):
        ...
        for row in rows:
            ...
            l.add_value('full_name', row['VISITANTE'])
            try:
                l.add_value('id_document', row['DOCUMENTO'].split(' ')[0])
            except:
                l.add_value('id_document', 'DNI')

            l.add_value('id_number', row['NUMERO_DOCUMENTO'])
            l.add_value('reason', row['MOTIVO'])
            l.add_value('entity', row['ENTIDAD'])
            l.add_value('host_name', row['FUNCIONARIO'])
            l.add_value('office', row['DEPENDENCIA'])
            l.add_value('time_end', row['HORA_SALIDA'])
            l.add_value('time_start', row['HORA_INGRESO'])
            l.add_value('location', row['SEDE'])
            l.add_value('host_title', row['DESCRIPCION_CARGO'])
