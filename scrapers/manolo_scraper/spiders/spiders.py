...
class SireviSpider(ManoloBaseSpider):
    ...
    ajax_page_pattern = '/index.php?r=consultas/visitaConsulta/updateVisitasConsultaResultGrid&ajax=lst-visitas-consulta-result-grid&lstVisitasResult_page=%s'  # noqa
    ...
    def get_item(self, data, date_str, row):
        ...
        loader.add_value('date', date_str)
        ...
        loader.add_xpath('host_title', './td[7]/text()')
...
class GobSpider(ManoloBaseSpider):
    ...
    def initial_request(self, date_start: str):
        ...
        token = driver.execute_script(
            "return grecaptcha.execute('6LdKTSocAAAAAN_TWX3cgpUMgIKpw_zGrellc3Lj', {action: 'create_comment'})"  # noqa
        )
        time.sleep(6)
        ...
        headers = {
            ...
            'sec-ch-ua-mobile': '?0',
            "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:91.0) Gecko/20100101 Firefox/91.0",  # noqa
    ...
    def get_item(self, item):
        ...
        if date_str:
            date_str = datetime.datetime.strptime(date_str, '%d/%m/%Y').strftime('%Y-%m-%d')  # noqa
